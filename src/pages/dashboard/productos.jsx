...
export function Productos() {
  ...
  const [productosTallaList, setProductosTallaList] = useState([]);
  const [productoColoresList, setProductoColoreslist] = useState([]);
  ...
  const URLProductoTallas = "http://localhost:8080/api/productotallas";
  const URLProductoColores = "http://localhost:8080/api/Productocolores";
  ...
  const getProductoTallas = async () => {
    try {
      const resp = await Axios.get(URLProductoTallas);
      setProductosTallaList(resp.data.productotallas);
    } catch (error) {
      console.log("Error al obtener los datos: ", error);
    }
  }

  const getProductoColores = async () => {
    try {
      const resp = await Axios.get(URLProductoColores);
      setProductoColoreslist(resp.data.productocolores);
    } catch (error) {
      console.log("Error al obtener los datos: ", error);
    }
  }
  ...
  const postProducto = () => {
    ...
    if (!nombre) {
      ...
      Axios.post(URLProductos, {
        ...
        showAlert("success", "producto registrada.");
        
          const id = productosList.filter(tll => tll).map(tll => tll.IdProducto).reverse()[0];

          setTimeout(() => {
            Axios.post(URLProductoTallas, {
              IdProducto: id,
              IdTalla: tallas,
              Cantidad: cantidad,
            }).then(() => {
              console.log("Talla registrada.");
            }).catch((error) => {
          ...
          setTimeout(() => {
            Axios.post(URLProductoColores, {
              IdProducto: id,
              IdColor: colores,
              Cantidad: cantidad,
            }).then(() => {
              console.log("Color registrado.");
            }).catch((error) => {
              console.error("Error al registrar el color:", error);
            });
          }, 1000); // 500ms después del registro de la talla
  ...
  const handleAddTalla = () => {
    setTallas([...tallas, tallaInput]);
    setTallaInput("");
  };

  const handleAddColor = () => {
    setColores([...colores, colorInput]);
    setColorInput("");
  };
  //delete
              ...
              >
                <div className="px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                  <Card className="">
                    ...
                    <h2 className=" max-w- text-sm py-2 text-gray-500">Información del producto</h2>
                    <CardBody className="px-2 pt-0 pb-2">
                        ...
                        <div className="col-span-2">
                          <div className="items-center justify-center">
                            <div className="flex items-center justify-center w-full">
                              <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-24 border-2  border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600" >
                      ...
                      <div className="grid grid-cols-2 gap-1 ">
                          ...
                          <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-9 sm:align-middle sm:max-w-lg sm:w-full">
                            <div className="px-1 py-2 sm:px-2">
                              <p className="mt-1 max-w-2xl text-sm text-gray-500">Complete los detalles de la talla</p>
                            </div>
                              ...
                              <div className="grid grid-cols-1 gap-4">
                                <div className="grid grid-cols-3 gap-4">
                                  ...
                                  <select
                                    ...
                                    {tallasList.map((tallas) => (
                                      <option key={tallas.IdTalla} value={tallas.IdTalla}>
                                        {tallas.NombreTalla}
                                      </option>
                                    ))}
                                  </select>
                                  ...
                                  <Button className=" bg-green-500 hover:bg-green-800 text-white font-bold  px-1 rounded" onClick={handleAddTalla}>Agregar</Button>
                                </div>
                                <div className="mt-4">
                                <Input
                                  label="Tallas "
                                  value={`Tallas: ${tallas.join(", ")} `}
                                  readOnly
                                />
                              </div>
                              </div>
                          ...
                          <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                            <div className="px-1 py-2 sm:px-2">
                              <p className="mt-1 max-w-2xl text-sm text-gray-500">Complete los detalles del color</p>
                            </div>
                              ...
                              <div className="grid grid-cols-1 gap-4">
                                <div className="grid grid-cols-3 gap-4">
                                  ...
                                  <select
                                    id="color"
                                    value={color}
                                    onChange={(event) => setColores(event.target.value)}
                                    className="block w-full pt-4  border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm ">
                                  ...
                                  <Button className="col-span-1 bg-green-500 hover:bg-green-800 text-white font-bold py-2 px-4 rounded" onClick={handleAddTalla}>Agregar</Button>
                                </div>
                                <div className="mt-4">
                                <Input
                                  label="Colores "
                                  value={`Colores: ${colores.join(", ")} `}
                                  readOnly
                                />
                              </div>
                              </div>
                      ...
                      <div className="flex justify-end items-center mt-3">
                        ...
                        <button onClick={() => setOpen(false)} className="btnCancel ml-3 text-white font-bold py-1 px-3 rounded">
                          Cancelar
                        </button>
                      </div>
                    </CardBody>
